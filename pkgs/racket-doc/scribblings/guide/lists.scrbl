...
@bold{译注}：@racket[foldl] 的参数分为三部分：第一部分为一个函数 @italic{proc}，
其参数个数等于要迭代的列表个数加一；第二部分为环境中初始的“当前值” @italic{init}；
第三部分是一个或多个等长的列表 @italic{lst ...+}。在本例中，匿名函数的第一个参数
@italic{elem} 会在每次迭代时绑定到列表中的每个元素，而最后一个参数 @italic{v}
