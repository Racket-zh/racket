...
@trans-note["Oling Cat"]{@racket[foldl] 的参数分为三部分：第一部分为一个过程
@italic{proc}，其参数个数等于要迭代的列表个数加一；第二部分为环境中初始的“当前值”
@italic{init}；第三部分是一个或多个等长的列表 @italic{lst ...+}。在本例中，
匿名函数的第一个参数 @italic{elem} 会在每次迭代时绑定到列表中的每个元素，
而最后一个参数 @italic{v} 则总是会绑定到“当前值”；待第一次迭代结束后，
匿名函数的返回值则作为下一次迭代的当前函数，以此类推。
将“当前值”绑定到匿名函数中最后一个参数的做法叫做递延式（Continuation-Passing
Style，旧译为“延续传递风格”）。}
